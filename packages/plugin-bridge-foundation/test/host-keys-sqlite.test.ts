...
describe('SqliteHostKeyRepo — markview drop-in compatibility', () => {
  it('custom tableName respects existing schema', async () => {
    ...
    repo.ensureSchema()
    const tableInfo = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as Array<{
      name: string
    }>
  ...
  it('CREATE TABLE IF NOT EXISTS is no-op on pre-existing table (markview migration v5 simulation)', async () => {
    ...
    db.prepare(
      `INSERT INTO host_keys (host_id, public_key_pem, status, fingerprint, registered_at, approved_at, last_used_at, relay_url)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run('mv-host', 'pem', 'active', 'fp', '2026-01-01T00:00:00Z', '2026-01-01T01:00:00Z', '2026-05-01T00:00:00Z', 'ws://127.0.0.1:3300/relay')
    ...
    const raw = db.prepare(`SELECT last_used_at, relay_url FROM host_keys WHERE host_id = ?`).get('mv-host') as {
      last_used_at: string
      relay_url: string
    }
