...
import { buildHostRecordStatus, HostKeyRegistry, verifyAuthorizationHeader, } from './auth/index.js';
...
import { staticUiHandler } from './endpoints/static-ui.js';
import { computeManifestHash } from './manifest/hash.js';
import { Counter, Gauge, MetricsRegistry } from './observability/index.js';
/**
...
export function createBridgeApp(opts) {
    const app = new Hono();
    ...
    let httpCounter = null;
    if (obs?.registry) {
        httpCounter = obs.registry.register(new Counter('plugin_bridge_http_requests_total', 'Total HTTP requests handled by plugin-bridge by method, path, status', ['method', 'path', 'status']));
        const startedAt = Date.now();
        obs.registry.register(new Gauge('plugin_bridge_uptime_seconds', 'Uptime of the plugin-bridge process in seconds', [], () => (Date.now() - startedAt) / 1000));
        obs.registry.register(new Gauge('plugin_bridge_host_registry_size', 'Number of registered hosts in the bridge HostKeyRegistry', [], 
    ...
    app.post('/plugin-bridge/v1/register-host', async (c) => {
        ...
        const providedOptionalFields = Object.keys(req).filter((k) => k !== 'host_id' &&
            k !== 'public_key_pem' &&
            k !== 'public_key' &&
            req[k] !== undefined);
        ...
        const missingFields = opts.registry.optionalFields.filter((f) => !providedOptionalFields.includes(f));
...
function authMiddleware(registry) {
    return async (c, next) => {
