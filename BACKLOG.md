...
|---|---|---|
| **Plugin-rollout** (manifest-filename + bundle + lifecycle) | agent (host meta-sidecar fetch/persist) · nexus (register `bytes`+`bundle_meta_url`) · cad2d (v1.2 e2e) | ✅ **plug-tmpl side COMPLETE + cross-repo-verified.** `bridge-foundation@0.12.0` `discoverManifest` + `create-plugin@0.8.1` (manifest.<id>.yaml scaffold, deterministic `bundle.tgz` packer, `bundle.launch.json` launch-contract, env-first port, bare-import guard). agents Host (`10f7c90`/`7947fd2`) ran the seam **end-to-end green** on a plug-tmpl fixture (download→USTAR→Zod→`bun --no-install`→running→health 200). **Seam-Ruling #6065:** `bundle.meta.json` is a correct **external sidecar** (self-referential sha/bytes) — plug-tmpl unchanged; host fetches `bundle_meta_url`, identity-gates vs catalog `{id,version,sha256,bytes}`, persists canonical, reads `launch`. **Awaiting others:** agent's meta-sidecar host-fix (in progress, pre-deploy), nexus register `bytes`+`bundle_meta_url`, cad2d v1.2. |
