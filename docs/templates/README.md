...
| [`SQLITE-SCHEMA-TEMPLATE.md`](./SQLITE-SCHEMA-TEMPLATE.md) | Optional — wenn Plugin SQLite nutzt (siehe `@nexus-mindgarden/plugin-storage-foundation`). |
...
- `@nexus-mindgarden/plugin-bridge-foundation` — HTTP-Server + Auth + Manifest
- `@nexus-mindgarden/plugin-storage-foundation` — SQLite + Migrations + Multi-Host-Paths
- `@nexus-mindgarden/plugin-svelte-foundation` — Custom-Element + Theme + Bundle-Config
- `@nexus-mindgarden/plugin-mcp-foundation` — Tool-Registry + Scope-Validation + Naming
