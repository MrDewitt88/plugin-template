...
> **Last update:** 2026-06-06 (Bun-storage session)
...
|---|---|---|---|
| 1 | Theseus register-host (a) `relay_url`/`host_version`-key-fix | agent | Triggered by Foundation v0.7.2 (c) — already shipped. Standby for wire-abgleich; agent ships (a) byte-aligned against 0.7.2. (b) backoff+cap already live on Theseus main `c2568db`. |
| 2 | Describe-Mind Stufe-5 Bun-Wire-up | oracle | Building against storage-foundation@0.7.0 `bun:sqlite` path. Standby for `bun:sqlite` PRAGMA / WAL-locking edge-cases (oracle #4667 flagged). |
| 3 | `granite-pilot-runner` chunking runtime-impl | wiz-mind | After their cycle picks up `toolCountPolicy` from granite-test v0.0.7 + ships runtime |
| 4 | v8-fam Pass-4 prediction-check | v8-fam | K-knob cohort (RFC §3.6: family-domain K=5 ≥60% lower-bound) |
| 5 | v8-corp K=10 plateau-stability check | v8-corp | Post-cap-adoption re-run (RFC §1.5: predicted stable 72.3%) |
| 6 | mind-canva sovereign env-free wire-up | mind-canva | adoption of Foundation `createHandshakeTokenStore` + `EcosystemAgentProvider` swap → drop `MC_AGENT_TOKEN` env-var |
| 7 | apex2d image-tools E2E test | apex2d | agent's rebuild of installed myMind to pick up `image.*` allowlist |
| 8 | plug-elec Pass-3 cohort-2c results | plug-elec | DEPRECATED-IN-PLACE re-run for R-12.b, L3-graduation flag for R-12.c |
...
1. **First action:** read chatbus (especially: agent shipping (a) against 0.7.2; oracle Describe-Mind Bun wire-up feedback; v8-fam/v8-corp Pass-4 results).
2. **If oracle reports Bun wire-up snags:** likely `bun:sqlite` PRAGMA/WAL edge-cases — may need a storage-foundation patch (see Notes/debt).
3. **If agent confirms (a) shipped:** Drift #105 chain fully closed (a+b+c) — note it in conventions/CHANGELOG.
4. **If a new Bun-plugin appears:** consider item #15 (a `bun:sqlite` openConnection convenience) if demand ≥2.
