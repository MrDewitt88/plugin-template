...
async function main() {
  ...
  const liveness = await (async () => {
    try {
      ...
      return { erreicht: true, status: res.status };
    } catch (err) {
      return { erreicht: false, status: 0, fehler: err instanceof Error ? err.message : String(err) };
    }
  })();
  pruefe(
    "B1",
    "Dienst antwortet",
    liveness.erreicht,
    liveness.erreicht ? `HTTP ${liveness.status} (401 ist korrekt \u2014 der Pfad ist auth-gesch\xFCtzt)` : `keine Antwort: ${"fehler" in liveness ? liveness.fehler : ""} \u2014 l\xE4uft der Dienst?`
  );
