 ...
 */
export function rollbackTo(db, migrations, targetId) {
    ensureMigrationsTable(db);
    const applied = appliedIds(db);
    ...
    for (const mig of sorted) {
        ...
        if (!applied.has(mig.id))
            continue;
        if (!mig.down) {
            throw new StorageError('rollback_blocked', `migration '${mig.id}' is non-reversible (down: null) — cannot rollback past it`, { migration_id: mig.id, target_id: targetId });
        }
        db.transaction(() => {
            mig.down(db);
            del.run(mig.id);
 ...
 */
export function listApplied(db) {
    ensureMigrationsTable(db);
    const rows = db
